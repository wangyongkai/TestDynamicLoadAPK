...
public class DLPluginManager {
     ...
     * @return //  dexPath /storage/emulated/0/DynamicLoadHost/plugin-debug.apk
     */
...
//    parent：父加载器
    ...
    private DexClassLoader createDexClassLoader(String dexPath) {
        ///data/user/0/com.king.testdynamicloadapk/app_dex
        File dexOutputDir = mContext.getDir("dex", Context.MODE_PRIVATE);
        dexOutputPath = dexOutputDir.getAbsolutePath();
        DexClassLoader loader = new DexClassLoader(dexPath, dexOutputPath, mNativeLibDir, mContext.getClassLoader());//注意此处传的父类加载器是PathClassLoader
    ...
    public int startPluginActivityForResult(Context context, DLIntent dlIntent, int requestCode) {
        ...
        //获取插件中的mainactivity
        String className = getPluginActivityFullPath(dlIntent, pluginPackage);
        //className = "com.king.plugin.MainActivity"; 设置第一次启动插件第一个界面
    ...
    //从插件的第一个界面跳转到第二个界面
    public int startPluginActivityForResultInner(Context context, DLIntent dlIntent, int requestCode) {
        if (mFrom == DLConstants.FROM_INTERNAL) {
            dlIntent.setClassName(context, dlIntent.getPluginClass());
            performStartActivityForResult(context, dlIntent, requestCode);
            return DLPluginManager.START_RESULT_SUCCESS;
        ...
        Class<?> clazz = loadPluginClass(pluginPackage.classLoader, "com.king.plugin.Main2Activity");
        if (clazz == null) {
            return START_RESULT_NO_CLASS;
        }
        ...
        // put extra data
        dlIntent.putExtra(DLConstants.EXTRA_CLASS, "com.king.plugin.Main2Activity");
        dlIntent.putExtra(DLConstants.EXTRA_PACKAGE, packageName);
        dlIntent.setClass(mContext, activityClass);
        performStartActivityForResult(context, dlIntent, requestCode);
        return START_RESULT_SUCCESS;
    ...
    private Class<?> loadPluginClass(ClassLoader classLoader, String className) {
        ...
        try {
            ...
            //   clazz1 = Class.forName("com.ryg.dynamicload.DLBasePluginActivity", true, classLoader);
     ...
     * @return
     */
    //如果DLBasePluginActivity也是用插件的apk的dexclassloader加载的 那么宿主中还有一份自己加载的DLBasePluginActivity ？
    private Class<? extends Activity> getProxyActivityClass(Class<?> clazz) {
        Class<? extends Activity> activityClass = null;
        if (DLBasePluginActivity.class.isAssignableFrom(clazz)) {//如果DLBasePluginActivity.class是宿主中的 而clazz是自定义加载器加载的 两个可以有继承关系吗？
